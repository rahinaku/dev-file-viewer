...
export function FileItem({ item, onImageClick, onFileClick }: FileItemProps) {
  const handleClick = () => {
    console.log('FileItem clicked:', { isImage: item.isImage, isVideo: item.isVideo, isAudio: item.isAudio, hasImageCallback: !!onImageClick, hasFileCallback: !!onFileClick, name: item.name });
    if ((item.isImage || item.isVideo || item.isAudio) && onImageClick) {
      onImageClick(item);
    } else if (onFileClick) {
      onFileClick(item);
    }
